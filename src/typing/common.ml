...
module MetaInfo = struct
	...
	let to_string = function
		...
		| Comma -> ":comma",("Used internally to mark a block that can be represented using a comma",[Internal])
		...
		| HxCNoHeader -> ":hxcNoHeader",("Prevents header fields from being generated for a class",[UsedOn TClass])
		| HxCSpecialize -> ":hxcSpecialize",("generate specialization functions for type parameters, internal",[UsedOnEither[TClass;TClassField]])
		...
		| Plain -> ":plain",("Generate a class field unqualified",[UsedOn TClassField;Platform C])
