...
type error_msg = Parser.error_msg =
	| Unexpected of token
	| Duplicate_default
	| Missing_semicolon
	| Unclosed_macro
...
exception Error = Parser.Error  (* of error_msg * pos *)
exception TypePath = Parser.TypePath (* of string list * (string * bool) option * bool (* in import *) *)
exception Display = Parser.Display (* of expr *)
...
and expr_next ?(in_case=false) e1 =
	let expr_next = (expr_next ~in_case:in_case) in parser
...
and parse_switch_cases eswitch cases = parser
	| [< '(Kwd Default,p1); '(DblDot,_); s >] ->
		...
		l , Some b
	| [< '(Kwd Case,p1); el = psep Comma (expr ~in_case:true); eg = popt parse_guard; '(DblDot,_); s >] ->
		(match el with
